...
use advent::{utils::file_reader::FileReader, model::stack::{Supplies, MoveCrate}};
...
fn test_day_5() {
    ...
    let (supplies, moves) = input.split_once("\n\n").expect("Cannot split by double newline");
    let mut supplies = Supplies::from_str(supplies).expect("Cannot parse supplies");

    let moves = moves.lines().map(|line| MoveCrate::from_str(line).expect("Cannot parse movecrate"));
